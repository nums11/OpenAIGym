...
class ExpectedSarsaAgent(FrozenLakeAgent):
  def train(self, env, num_episodes):
    ...
    for episode in tqdm(range(num_episodes)):
      ...
      for step in range(max_steps_per_episode):
        ...
        q_table[state][action] = q_table[state][action] + learning_rate * (
          reward + discount_rate * expectation - q_table[state][action])
        state = new_state
    ...
    self.savePolicyFromQTable(q_table, 'expected_sarsa_policy_100k_min03')
